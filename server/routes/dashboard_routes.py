...
from services.async_ticket_service import AsyncTicketService
...
async def get_tickets(
    ...
    try:
        result = await AsyncTicketService.get_tickets(
            status=status,
            limit=limit,
            offset=offset,
            search=search
...
async def get_ticket(
    ...
    try:
        ticket = await AsyncTicketService.get_ticket_by_id(ticket_id)
        return ticket
...
async def update_ticket_status(
    ...
    try:
        result = await AsyncTicketService.update_ticket_status(
            ticket_id=ticket_id,
...
async def update_ticket_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to assign ticket status")
...
async def assign_ticket(
    ...
    try:
        result = await AsyncTicketService.assign_ticket(
            ticket_id=ticket_id,
...
async def get_analytics(
    ...
    """Get ticket details with caching"""
    try:
        analytics = await AsyncTicketService.get_analytics(days=days)
        return analytics
    except ValidationError as e:
        raise HTTPException(status_code=400, detail=e.message)
    except Exception as e:
